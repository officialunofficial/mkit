...
//! | `refs.non_refs_prefix_rejected` | | a grammar-valid name outside `refs/` is `invalid_argument` on `ReadRef`, `UpdateRef`, `AdvanceRefs`, and neither `ListRefs("")` nor a listing of its parent shows it (SPEC-REFS v3 §2) |
