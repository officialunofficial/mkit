//! Integration tests for refs + index + worktree + ignore +
//! `repo_lock`.
//!
...
//! temp `ObjectStore`-rooted repo, which is the bit the higher-level
//! ops layers depend on.
//!
//! The goldens themselves are generated by the example
//! `examples/generate_refs_index_goldens.rs`; re-running the generator
...
fn read_golden(name: &str) -> Vec<u8> {
    ...
    fs::read(&path).unwrap_or_else(|e| {
        panic!(
            "failed to read golden vector {}: {e}. Run `cargo run -p mkit-core --example generate_refs_index_goldens`.",
            path.display()
        )
    })
}
