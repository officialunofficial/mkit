...
fn generate_secp256k1_signer()
-> Result<(mkit_attest::signer_k256::Secp256k1Signer, [u8; 32]), String> {
    for _ in 0..256 {
        let mut buf = [0u8; 32];
...
fn generate_p256_signer() -> Result<(mkit_attest::signer_p256::P256Signer, [u8; 32]), String> {
    for _ in 0..256 {
        let mut buf = [0u8; 32];
        getrandom::fill(&mut buf).map_err(|e| format!("rng failed: {e}"))?;
...
/// Write `secret` to `path` atomically with mode 0600 on Unix.
///
/// Permissions are set via the open `File` handle (not path-based
/// `set_permissions`) so we cannot be tricked into chmod-ing a
/// different inode mid-write — the same hardening `mkit-core::sign`
/// applies to `default.key` (finding H3).
fn write_raw_32(path: &Path, secret: &[u8; 32]) -> std::io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        f.write_all(secret)?;
        f.sync_all()?;
        // Tighten to 0600 via the open handle in case the file pre-existed
        // with a wider mode — closes the TOCTOU window.
...
/// Load a raw 32-byte secret with the same permission checks the
/// repo-key signer enforces on non-Ed25519 key files.
fn load_raw_32(path: &Path) -> Result<[u8; 32], u8> {
