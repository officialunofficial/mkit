...
//!   **Not** auto-generated as of 0.3.0 — the caller must run
...
fn build_repo_key_signer(
    ...
    match algorithm {
        Algorithm::Ed25519 => {
            // Reuse the commit signer's key path. As of 0.3.0 we no
...
mod tests {
    ...
    /// 0.3.0: `mkit attest --signer repo-key` no longer auto-generates
