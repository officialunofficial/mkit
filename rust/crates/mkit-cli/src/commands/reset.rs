...
pub fn run(args: &[String]) -> u8 {
    ...
    // Run the same dirty/untracked guard as `checkout` BEFORE any
    // mutation, unless `-f`. It refuses if discarding would lose
    // locally-modified, staged, or colliding-untracked content — an mkit
    // safety divergence (git's `reset --hard` discards silently).
    if opts.hard
        && !opts.force
        && let Err(e) =
    ...
    // For --hard, capture the tracked paths the target DROPS, computed from
    // the current index BEFORE it is re-synced. `clean = false` won't
    // delete these, so we remove them from the worktree ourselves — git
    // `reset --hard` removes tracked files absent from the target while
    // leaving untracked files untouched.
    let hard_removed: Vec<String> = if opts.hard {
...
/// tree — for `--hard` these worktree files are deleted (git removes
/// tracked files the target drops; `restore_tree_to_worktree` with
/// `clean = false` writes/overwrites but never deletes).
