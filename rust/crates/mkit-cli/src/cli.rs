...
commands:
  ...
  stash pop [<n>|stash@{n}]    Apply and remove a stash entry (default 0)
  stash apply [<n>|stash@{n}]  Apply a stash entry without removing it (default 0)
  ...
  cherry-pick [-n] [-m <msg>] <hash> | --continue | --abort
                    Apply a commit to the current branch (-n stages the change
                    without committing; -m overrides the message)
