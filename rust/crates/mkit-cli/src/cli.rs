...
commands:
  ...
  export-legacy [--key <path>] [--json] <src> <dst>
                    Translate a pre-merkle (incompatible-format) repository
                    at <src> into a fresh current-format repository at
                    <dst>, re-signing translated commits/tags with a
                    dedicated export key; <src> is read-only
