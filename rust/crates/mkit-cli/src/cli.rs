...
commands:
  ...
  pull [<remote>] [--all]  Pull changes from remote (fast-forward; prints
                    `Updating <a>..<b>`/`Fast-forward`/diffstat, or `Already
                    up to date.`); --all pulls every configured remote
  fetch [<remote>] [--all]  Download from remote without merging (prints
                    `From <url>` + per-ref summary; silent when nothing
