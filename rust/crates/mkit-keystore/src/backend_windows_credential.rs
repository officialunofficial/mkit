...
/// User-scoped Windows Credential Manager backend.
#[derive(Clone, Debug, Default)]
pub struct WindowsCredentialKeystore;

impl WindowsCredentialKeystore {
    /// Create a Windows Credential Manager backend instance.
    #[must_use]
...
impl Keystore for WindowsCredentialKeystore {
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            backend: BackendKind::WindowsCredentialManager,
...
impl KeyGenerator for WindowsCredentialKeystore {
    fn generate(
        &self,
        label: &KeyLabel,
        algorithm: Algorithm,
...
impl KeyImporter for WindowsCredentialKeystore {
    fn import(
        &self,
        label: &KeyLabel,
        secret: SecretKey,
...
impl KeyOpener for WindowsCredentialKeystore {
...
impl KeyLister for WindowsCredentialKeystore {
...
impl KeyExporter for WindowsCredentialKeystore {
...
impl KeyDeleter for WindowsCredentialKeystore {
...
fn map_keyring_error(error: keyring_core::Error, label: &KeyLabel, algorithm: Algorithm) -> Error {
    match error {
        ...
        keyring_core::Error::NotSupportedByStore(error) => Error::BackendUnavailable(format!(
            "Windows Credential Manager operation unsupported: {error}"
        )),
        other => Error::Io(format!("Windows Credential Manager: {other}")),
    }
}
...
fn keyring_backend_error(operation: &str, error: keyring_core::Error) -> Error {
    match error {
        ...
        keyring_core::Error::NotSupportedByStore(error) => {
            Error::BackendUnavailable(format!("Windows Credential Manager {operation}: {error}"))
        }
        other => {
            Error::BackendUnavailable(format!("Windows Credential Manager {operation}: {other}"))
        }
    }
}
