...
- Codegen: `build.rs` runs `connectrpc-build` directly over the **canonical**
  proto &mdash; `apps/repo-worker/proto/mkit/repo/v1/repo.proto` (referenced by a
...
so every scalar field has explicit presence &mdash; the Rust side wraps/unwraps the
...
`ed25519_sign` and `blake3_hex`); this client only forwards the resulting headers.
...
| `X-Created-At`   | `String(epochMillis)` &mdash; **epoch ms**, NOT ISO-8601                |
| `Idempotency-Key`| opaque dedupe token (omitted ⇒ canonical field is `""`)           |

These exactly match `apps/repo-worker/src/envelope.rs`'s `EnvelopeHeaders`
(mirrored on the JS side in `apps/web/src/lib/repo/envelope.ts`). The server
recomputes `BLAKE3(rawBody)` and rejects on
...
This is a plain envelope digest &mdash; the SPEC-SIGNING commit/remix/tag domain
prefixes do **NOT** apply.

### Which bytes are hashed &mdash; and why the client owns the digest

`bodyDigest` (and the server's `actualBodyDigest`) is `BLAKE3` of the
**serialized protobuf request message** &mdash; the exact bytes on the Connect/proto
...
3. It invokes `sign(bodyDigestHex)` &mdash; JS builds the canonical string with that
...
serialization (for example JS `JSON.stringify`) would fail `X-Digest` verification.
