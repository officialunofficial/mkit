...
_mkit_complete() {
    ...
    case "${words[1]}" in
        ...
        worktree)
            ...
            ;;
        workspace)
            COMPREPLY=( $(compgen -W "create status diff add log commit export push abandon --bundle --hosted --base --path --accept-bundle-selection --ref --workspace-id --grant-id --grant-generation --cached --all --message -m --author --output --endpoint --repository --candidate --acknowledge-possible-publication --format --help" -- "$cur") )
            return
            ;;
        stash)
