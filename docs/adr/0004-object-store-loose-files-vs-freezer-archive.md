# ADR 0004 — Object store stays loose files; `Freezer`/`Archive` rejected on GC-unlink grounds
...
v2026.5.0 — already a dependency for the commit-history MMR (`history.rs`) and the
vendored-BMT cross-check (`merkle.rs`) — ships two write-once key-value primitives at
...
storage wherever there is a reasonable fit — defaulting to migration unless the spike
...
produces — a 15-byte blob, a 64 KiB blob, a tree, and two commits), then exercises:

1. **`freezer_put_get_byte_identical`** — puts all five objects into a `Freezer`
...
2. **`freezer_has_no_per_key_delete`** — documents, and asserts the observable
...
3. **`archive_prune_deletes_a_prefix_not_a_single_index`** — the concrete
   ...
   `archive::Archive` trait — see `archive/mod.rs`'s trait definition, which has no
   ...
   Re-inserting index 0 afterward is rejected (`Error::AlreadyPrunedTo`) — a pruned
...
store that isn't in the live set — via `store.remove_object(&h)`, one `fs::remove_file`
...
- **`Freezer`** has no deletion primitive at all short of destroying the entire
  ...
  modified" and "avoids ever rewriting (i.e. compacting) inserted data" — this is
  not an oversight, it is the design. There is no `Freezer` configuration or
  companion type that adds per-key removal.
- **`Archive`** (the prunable variant — the only one with any removal capability)
  ...
  last N blocks, drop everything older" — see the module doc's "Querying for Gaps"
  ...
  expressible — `prune` always takes a *prefix*, never an arbitrary subset.
...
swap. That is a fundamentally different GC architecture — compaction-based instead
of unlink-based — with materially different cost (proportional to *retained* data,
...
repo size — only that it's a different, larger commitment than a store swap).
...
- **Sync/async bridge cost.** `commonware-storage` is async-over-tokio;
  ...
  thread). This is a known, already-amortized cost class, not a novel one — it would
...
- **wasm target support.** Verified by grep: `mkit-wasm/src/**` has zero references
  ...
  **non-issue** for this decision — consistent with the issue's own hint to verify
  rather than assume.
...
- **Built-in zstd compression.** `Freezer`/`Archive` both support per-value zstd
  ...
  — but it is available independently of the store's file-vs-journal question (see
  Consequences).
...
- **#646 (pack payload compression) is *not* resequenced behind a migration**,
  because there is no migration. The original resequencing concern — that
  `Freezer`/`Archive`'s built-in zstd might reshape #646's scope — does not
  ...
  of this ADR's GC finding, that is a separable, much smaller change (e.g.
  compressing loose-file bytes before the atomic rename) that doesn't require
  adopting `Freezer`/`Archive`'s journal model — and is out of scope here.
...
- **If this decision is ever revisited**, a real migration issue would need to
  scope at least: (a) which GC architecture replaces mark-and-sweep-with-unlink —
  ...
  absent the GC blocker — the spike used `Archive` only to test prune semantics;
