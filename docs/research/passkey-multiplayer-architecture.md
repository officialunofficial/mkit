...
shared, Cloudflare-hosted mkit repo (D1/KV/R2) — a *multiplayer sandbox*.
...
| Signing | **Ed25519** | Signs mkit commits (core is Ed25519-only) | **Derived from the passkey via PRF — never stored** |
...
only in memory — there is no key file. Same passkey → same Ed25519 pubkey →
...
- **Footgun (moot here):** deleting the passkey loses the derived key. For a
  demo this is fine — ephemeral identity, no stakes. (For production you'd
...
1. **Enroll** — `navigator.credentials.create({ pubKeyCredParams:[{alg:-7}], rp:{id:"mkit.sh"}, extensions:{prf:{}} })`; confirm `getClientExtensionResults().prf.enabled`.
2. **Derive** — `navigator.credentials.get({ extensions:{ prf:{ eval:{ first: SALT }}}})` → PRF → HKDF → Ed25519 seed → pubkey. (A `get()` is required to actually obtain PRF output on most platforms.)
3. **Author + sign** — build the commit in WASM: `blob_encode` / `tree_encode` / `commit_encode_and_sign(tree, parents, msg, ts, seed_hex)` (all already exported). Output = canonical object bytes + commit hash + signature.
4. **(optional) Attest the binding** — P-256 passkey signs a DSSE attestation over the Ed25519 pubkey; verify via `verify_webauthn_wrapping_with_policy` (added on this branch). Shows the full passkey signing lifecycle.
5. **Push** — sign the request envelope (Ed25519) and POST objects + CAS-advance the ref (§3).
6. **Watch** — subscribe to the repo's live ref stream for other players' commits (§5).
...
won't wrap PRF, so call the raw API). No Rust HTTP transport in the browser —
plain `fetch`.
...
- **Zustand — client identity/session only.** The passkey credential id, the
...
| Packfile creation in WASM | ❌ not exposed — **avoided** by using loose `PUT /objects/{hash}` |
