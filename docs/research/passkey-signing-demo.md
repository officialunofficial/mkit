...
Question: can we let a visitor on the demo pages enroll a **passkey** (Touch ID /
...
This is not a limitation to fight — it's the correct framing for the demo. The
...
If we ever want passkeys to sign *commits themselves*, that needs a spec change
(a P-256 commit-signature variant or a webauthn commit domain) — out of scope
...
> **Status (this branch):** step 3a is **done** — `attest_pae`,
...
### 3a. Two WASM exports (`rust/crates/mkit-wasm/src/lib.rs`) — DONE
...
- `verify_webauthn_wrapping_with_policy(..., policy_json) -> void` (empty string = permissive; recognises `expected_rp_id`, `allowed_origins`, `require_user_presence`, `require_user_verification`, `allow_cross_origin`, `previous_sign_count`)
...
`wevm/webauthx`) — it already does COSE→SEC1 key extraction, the `create`/`get`
...
orchestrator (client `Registration.create`/`Authentication.sign`); we likely only
need its client half since **mkit's WASM is the verifier** — its server half is
...
mkit's verifier requires `clientDataJSON.challenge == base64url-nopad(PAE)` — i.e.
...
Two options:
- **A — keep demo payloads small** (tiny claim JSON). Zero code change; works with
  the current verifier as-is. Cleanest for a *demo*. **Recommended starting point.**
- **B — add a hashed-challenge verifier variant** (`challenge == base64url(SHA-256(PAE))`)
  for production robustness. This is a SPEC-EXTERNAL-SIGNER / verifier change, not
  a demo concern — flag it, don't block on it.
...
## 5. On "we already have an Apple/Google org"
...
matching RP ID. The Apple Developer / Google Play org matters only if we later
extend to **native iOS/Android apps** sharing the same passkeys via
Apple App Site Association / Android Digital Asset Links — a separate, larger
...
No new cryptography, no spec change for option A. The risky/expensive part —
a correct, low-S, policy-checked WebAuthn verifier — is already written and tested.
