...
| `fuzz_targets/staged_update.rs` | borrowed MKWU v1 carrier/key and ordered inventory, with an unconditional committed-valid frame plus selected valid diff/file and invalid pair/file record, cursor, and context lanes; it does not prove global exact-set or durable completion |
