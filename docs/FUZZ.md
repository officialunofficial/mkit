...
| `fuzz_targets/partial_overlay.rs` | `partial::replace_files` (bounded structured replacement/reuse batches may succeed or reject; every success replays deterministically, preserves unselected entry triples and selected modes, and emits canonical objects under their type-dependent ids) |
