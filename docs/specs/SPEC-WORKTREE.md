...
Per-tree locking is insufficient to serialize two linked trees mutating the
same branch/tag/remote ref. Each ref mutation additionally takes the shared
full-ref guard specified in SPEC-CONCURRENCY §4, after any history guard.
Unconditional operations and deletes participate in this same order.
