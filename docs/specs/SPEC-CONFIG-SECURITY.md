...
| `remote.<name>.admission_headers` (M3, not yet implemented) | **UNSAFE** | Adds header names to the admission-helper allowlist of remote `<name>`. If repo-controlled, it could widen what a helper may attach to requests. It can never add a hard-reserved header (every `X-` header, `Host`, `Content-*`, `Transfer-Encoding`, `Connect-*`, `Cookie`, `Idempotency-Key`, hop-by-hop headers); an entry naming one has no effect. Unlike `remote.<name>.url`, this member of the `remote.<name>.*` family is user-scoped only. |
