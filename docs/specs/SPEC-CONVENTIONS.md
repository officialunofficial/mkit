...
# SPEC-CONVENTIONS — shared vocabulary for the SPEC-*.md corpus
...
| Bindingness | `normative` | Peers/implementations that don't conform are non-conformant — cross-process or cross-repo interop depends on this document. |
| | `advisory` | Describes local, non-exchanged behavior (e.g. an index or cache file no other peer ever reads); an implementation MAY diverge without breaking interop, though it still shouldn't. |
...
apply (e.g. this document itself). New or heavily-revised specs
...
non-urgent inconsistency — retrofitting them is left as a documentation
...
- **`normative`** documents describe cross-process or cross-repo wire
  state. A version bump here MUST either add a migration shim or be
  called out as an explicit, deliberate breaking change (see
  CONTRIBUTING.md's "spec changes are versioned" review bar).
- **`advisory`** documents describe local-only, unexchanged state
  (SPEC-INDEX is the canonical example). mkit does not maintain
  migration shims for its own local, unreleased state files — a reader
  that upgrades mkit and finds a stale local file simply gets a clean
  rebuild of it (e.g. `.mkit/index` is regenerated by the next `add`/
  `commit`); there is no installed base whose files must keep parsing.
...
- Every multi-byte integer is **little-endian** unless the document
  ...
  they implement, e.g. AWS SigV4 in SPEC-TRANSPORT — those documents
  say so explicitly).
- A **length-prefixed field** means `[u32 LE length][length bytes]`
  unless a document specifies a different integer width for that
  field. This applies even to fixed-size fields (e.g. a 24-byte nonce
...
mkit's signing and hashing domain separators (e.g. `mkit.tag\0`,
BLS namespace strings like `dsse/v1`) are permanent, literal byte
strings baked into the signing/hashing contract — not a version field
...
(one directory per format) and are the authoritative pinned bytes —
...
contract — a wire format, an algorithm, a predicate a verifier
evaluates — not as a reference to a specific library function or crate
