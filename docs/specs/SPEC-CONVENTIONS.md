...
| | `advisory` | Describes local, non-exchanged behavior (for example, an index or cache file no other peer ever reads); an implementation MAY diverge without breaking interop, though it still shouldn't. |
...
- **`advisory`** documents describe local-only, unexchanged state
  ...
  migration shims for its own local, unreleased state files &mdash; a reader
  that upgrades mkit and finds a stale local file simply gets a clean
  rebuild of it (for example, `.mkit/index` is regenerated by the next `add`/
