# mkit-sign-se &mdash; Apple Secure Enclave signer for mkit
...
- **P-256 only.** The Secure Enclave supports no other algorithm. Any
  ...
  `Error` frame carrying `ERROR_CODE_UNSUPPORTED_ALGORITHM` &mdash; this
...
- **Non-extractable private key.** `SecureEnclave.P256.Signing.PrivateKey`
  ...
  the blob persisted to the keychain is an encrypted handle that only
...
`mkit-sign-se sign` speaks the v1 wire &mdash; length-prefixed protobuf
...
runtime &mdash; [SwiftProtobuf](https://github.com/apple/swift-protobuf),
...
#719) &mdash; no vendored mkit checkout or hand-rolled `protoc` invocation
...
`common`/`signer` outputs into `Generated/` &mdash; no `protoc` or
...
The Secure Enclave key to sign with is selected by **tag** &mdash; the same
...
- Swift 5.9+ toolchain &mdash; shipping Xcode 14+ or the standalone
...
processes successive `Hello` and `SignRequest` pairs until the caller
closes the stream &mdash; a clean EOF on the length prefix is treated as a
graceful shutdown.
...
`.mkit/signing.key` &mdash; a *different* flow. For the SEP workflow:
...
- **Non-extractable secret.** The private scalar is generated inside
  the SEP and never leaves it. A full disk snapshot &mdash; or even a
  compromised kernel &mdash; cannot exfiltrate the key material; they can
  only ask the SEP to sign, subject to whatever access control was
  set at creation.
- **Biometric-gated signing.** With `--require-biometric`, signing
  prompts for Touch ID/Face ID. Canceling the prompt surfaces as
...
- **Device loss = key loss.** SEP keys are bound to the physical SEP
  ...
  wiped/reset device loses the key irrecoverably &mdash; you cannot back
...
- **One binary, one keychain.** All keys this binary creates live
  ...
  `dataRepresentation` blob (this binary doesn't; it deletes the keychain
  item wholesale).
...
&mdash; use `mkit-sign-file` (the reference signer) for dev/CI, and the
...
session (for example SSH, LaunchDaemon, cron). Unlock your keychain, or
...
Developer ID signature and notarization. Neither is needed for local
...
with a small python3 helper (no protobuf dependency &mdash; it encodes the
...
shell e2e &mdash; and so the path works on case-insensitive macOS
...
`mkit-sign-tpm`, `mkit-sign-ctap`) &mdash; same protocol, different
