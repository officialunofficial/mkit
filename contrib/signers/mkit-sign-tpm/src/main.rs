...
//! with a P-256 signing primary key held inside a TPM 2.0 device. This
//! is the Linux-native analog of the Apple Secure Enclave signer
//! (`mkit-sign-se`): one platform-local hardware-backed key, one argv
//! subcommand shape, one explicit "reject non-native algorithms" stance.
//!
//! Subcommands:
//!
...
//!
//! The `sign` subcommand is the one mkit drives: it reads one line of
//! JSON `{pae_base64, algorithm}` from stdin and writes one line of
//! JSON `{keyid, sig_base64}` to stdout. TPM-native signatures are
//! ASN.1 DER-encoded `(r, s)` pairs; we unpack them to the 64-byte
//! compact `r || s` the spec requires.
//!
...
fn main() -> ExitCode {
    ...
    match run(subcommand, rest) {
        ...
        Err(e @ SignerError::UnsupportedAlgorithm(_)) => {
            // Distinct exit code 2 for "wrong algorithm" — matches
            // mkit-sign-se. SPEC §5 treats any non-zero uniformly
            // today; reserving 2 makes a future mkit able to surface
            // "retry under a different algorithm" without parsing
            // stderr. Still write the reason to stderr per SPEC §5
            // recommendation ("one-line human-readable reason").
...
/// The hot path mkit drives. Kept as a free function so the argv
/// dispatch above stays readable.
fn run_sign(handle: u32) -> Result<(), SignerError> {
    // SPEC-EXTERNAL-SIGNER §6: cap stdin at 1 MiB. Read cap + 1 so we
    // can distinguish "exactly at cap" from "exceeded cap".
    ...
    if req.algorithm != "p256" {
        // Exit 2 per main's dispatch; message on stderr keeps a
        // human-readable trace for the user running mkit directly.
        return Err(SignerError::UnsupportedAlgorithm(req.algorithm));
    }
    ...
    let (pubkey_compressed, sig_compact) = tpm::sign(handle, &pae)?;
    // Re-assert invariants that the tpm shim should already satisfy;
    // cheap defence-in-depth against a future refactor that loosens
    // one of the shapes the spec requires.
    if pubkey_compressed.len() != 33 {
        return Err(SignerError::Internal(format!(
            "pubkey {} bytes, want 33",
            pubkey_compressed.len()
        )));
    }
    if sig_compact.len() != 64 {
        return Err(SignerError::Internal(format!(
            "signature {} bytes, want 64",
            sig_compact.len()
        )));
    }

    let keyid = format!("p256:{}", to_hex(&pubkey_compressed));
    let sig_b64 = B64.encode(&sig_compact);
    let stdout = std::io::stdout();
    writeln!(
        stdout.lock(),
        "{{\"keyid\":\"{keyid}\",\"sig_base64\":\"{sig_b64}\"}}"
    )
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedAlgorithm(a) => write!(
                f,
                "algorithm `{a}` not supported — TPM signer is P-256 only; request `\"algorithm\":\"p256\"`"
            ),
...
mod tests {
    ...
    fn request_parses_fields() {
        let raw = br#"{"pae_base64":"AAAA","algorithm":"p256"}"#;
        let req: Request = serde_json::from_slice(raw).unwrap();
        assert_eq!(req.pae_base64, "AAAA");
        assert_eq!(req.algorithm, "p256");
    }

    #[test]
    fn request_ignores_extra_fields() {
        // Forward-compat escape hatch per SPEC §9.
        let raw = br#"{"pae_base64":"AAAA","algorithm":"p256","v":2,"future_field":"x"}"#;
        let req: Request = serde_json::from_slice(raw).unwrap();
        assert_eq!(req.algorithm, "p256");
    }
