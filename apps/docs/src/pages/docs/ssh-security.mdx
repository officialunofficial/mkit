---
title: SSH transport security
description: "Trust model for the mkit+ssh:// transport: known_hosts handling, AuthorizedKeysCommand, forced commands, and recommended sshd_config."
---
...
What mkit's SSH transport does and does NOT do, what guarantees users
can rely on, and how to harden a deployment.
...
([Transport](/docs/spec/transport) §7). Nothing else. In particular, mkit does NOT:
...
1. User generates one Ed25519 key (`mkit keygen` — the seed doubles as
   an `id_ed25519` for OpenSSH 8.0+; see [Signing](/docs/spec/signing) §8).
