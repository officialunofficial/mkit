---
title: Architecture
description: "Code map of the mkit Rust workspace, data-flow between objects, packs, refs and transports, and the attestation pipeline."
---

# Architecture

:badge[Informative]{info}

A code map for contributors and integrators — where things live and
how the pieces connect. Normative shape lives in the
[specifications](/docs/spec/objects); this page is the index that
points at them.
...
defined in [External signer](/docs/spec/external-signer).
...
The contract is [External signer](/docs/spec/external-signer)
(Protocol v1, v1.1 adds optional WebAuthn wrapping). Shape:
...
[Objects](/docs/spec/objects). Update the matching golden vector under
...
trait, then read [Transport](/docs/spec/transport) for the seven verbs. Use
...
token), implement [External signer](/docs/spec/external-signer) and ship a binary —
...
- [Objects](/docs/spec/objects) — on-disk object format
- [Packfile](/docs/spec/packfile) — packfile wire format
- [Delta](/docs/spec/delta) — delta encoding
- [FastCDC](/docs/spec/fastcdc) — content-defined chunking
- [Refs](/docs/spec/refs) — ref names, CAS variants
- [Staging index](/docs/spec/staging-index) — repo-local index
- [Transport](/docs/spec/transport) — seven-verb wire protocol
- [Signing](/docs/spec/signing) — commit / remix signing
- [Attestations](/docs/spec/attestations) — DSSE + in-toto v1
- [External signer](/docs/spec/external-signer) — external signer subprocess protocol
- [SSH security](/docs/ssh-security) — SSH trust model (informative)
- [Threat model](/docs/threat-model) — security boundaries (informative)
- [Fuzzing](/docs/fuzz) — fuzz harness conventions
