...
Content-defined chunking of large files. Specifies gear-table
derivation, chunking parameters, and the determinism contract that
keeps `chunked_blob` hashes stable across producers.
...
a `chunked_blob` object ([Objects](/docs/spec/objects) §7).
...
transfers (see [Packfile](/docs/spec/packfile) §3.2 delta vs. [Objects](/docs/spec/objects) §7 chunked
...
See [Objects](/docs/spec/objects) §7 for the container. Key points for chunking:
...
- Each chunk is itself stored as a `blob` object ([Objects](/docs/spec/objects) §3)
