---
title: Refs
description: "Reference namespace, atomic CAS semantics, and on-disk layout for branch and tag pointers."
---
...
Ref names, ref wire bytes, ref storage layout, and the exact
...
| file      | atomic      | atomic (via `O_EXCL` create)          | **NOT atomic** — read-then-write race across processes         | v1 known gap; A future version will replace with lockfile. |
...
*memory/file CAS:* v1 ships the non-atomic implementations; [Transport](/docs/spec/transport)
