...
The schemas live in [`rust/crates/mkit-rpc/proto/`](https://github.com/officialunofficial/mkit/tree/main/rust/crates/mkit-rpc/proto/):

| File | Purpose |
|---|---|
| `common.proto` | Shared types: `Algorithm`, `KeyForm`, `ErrorCode`, `Error`, `ProtocolVersion` |
| `signer.proto` | External-signer protocol (`SignerFrame` oneof). See [External signer](/docs/spec/external-signer) for prose. |
| `ssh.proto` | SSH transport protocol (`SshFrame` oneof). See [Transport](/docs/spec/transport) for prose. |
...
- **Schema-as-source-of-truth.** The .proto files are the contract;
  prose specs (this file, [External signer](/docs/spec/external-signer), [Transport](/docs/spec/transport)) are
  derivative.
