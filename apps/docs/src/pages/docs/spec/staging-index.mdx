---
title: Staging index
description: "Repo-local staging index format used by mkit add, mkit rm, and mkit commit."
---

# Staging index

:badge[Local-only · v1]{tip}

The on-disk layout of `.mkit/index`, the working-tree staging area.
Local-only — never exchanged between peers.
...
pass [Objects](/docs/spec/objects) §4.1 name rules for each path segment except that
...
Executable bit (mode 0x04 in [Objects](/docs/spec/objects) §4.2) is tracked in v1 via a
