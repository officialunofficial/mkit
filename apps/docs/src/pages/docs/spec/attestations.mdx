---
title: Attestations
description: "Native in-toto v1 + DSSE attestation primitive for mkit: predicate types, multi-signer envelopes, and on-disk layout."
---
...
The native attestation primitive for mkit: how attestations are
produced, stored, transported, and verified, including the on-disk
layout, the wire envelope, the signing contract, the CLI, and the
transport additions.

Out of scope: what an attestation *means* — that's the business of
the predicate-type URI, which is chosen by whoever makes the claim
(mkit itself, Makechain, GitHub, a user's build server, and so on).
...
- **Hash** is BLAKE3, consistent with [Objects](/docs/spec/objects).
- **Signature** is Ed25519, consistent with [Signing](/docs/spec/signing) §1 —
...
[**[External signer](/docs/spec/external-signer)**](/docs/spec/external-signer). Protocol
...
### 7.2 Signing code ([Signing](/docs/spec/signing))
...
confusion (see [Signing](/docs/spec/signing) §2) is therefore extended
to cover attestations with no additional work.

### 7.3 Transport

[Transport](/docs/spec/transport) v1 gains **two** new verbs:
