...
The exact bytes an Ed25519 signature covers on a commit or remix,
and the domain separator that keeps the two signature spaces
disjoint.
...
For a commit C (as defined in [Objects](/docs/spec/objects) §5), the canonical signing
bytes are:
...
Where `PROLOGUE` is the 6 bytes defined in [Objects](/docs/spec/objects) §2:
...
3. `parent_count` (4 LE) and `parent_count` × `parent_hash` (32 each) —
   commit history.
4. `Identity author` (variable; see [Objects](/docs/spec/objects) §9) — length-prefixed,
...
These two fields are the only commit-struct fields excluded from
signing bytes.
...
For a remix R ([Objects](/docs/spec/objects) §6):
...
This is the defence against cross-domain confusion. The previous
scheme used only the ObjectType tag byte (0x03/0x04) as separator —
one byte of domain — and was fragile. v1 uses ≥ 11 bytes of
high-entropy ASCII plus a null terminator.
...
1. Check object prologue ([Objects](/docs/spec/objects) §2).
...
- **DSSE attestation signing** via the `repo-key` signer
  ([Attestations](/docs/spec/attestations) §6.2).
