...
For integrators shipping a signer for a platform mkit does not
ship natively — Apple Secure Enclave, Ledger / Trezor, WebAuthn/CTAP,
MetaMask / EVM wallet bridges, Makechain, HSMs, etc.

The wire format itself is defined in [RPC](/docs/spec/rpc). This
...
- The DSSE / in-toto envelope structure ([Attestations](/docs/spec/attestations)).
- Trust-roots configuration ([Attestations](/docs/spec/attestations) §7).
...
and stdout (signer → mkit). Per [RPC §1](/docs/spec/rpc#1-wire-framing).

Schema: [`rust/crates/mkit-rpc/proto/signer.proto`](https://github.com/officialunofficial/mkit/blob/main/rust/crates/mkit-rpc/proto/signer.proto).
...
| `payload` | Raw bytes to sign. For DSSE, this is the PAE per [Attestations](/docs/spec/attestations) §4. |
...
Error code semantics: see [RPC §3.3](/docs/spec/rpc#33-errorcode).
...
with a sibling `signer2.proto`. See [RPC §2](/docs/spec/rpc#2-versioning).
