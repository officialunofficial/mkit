...
:badge[Normative · v1]{success}

The contents of `.mkit/objects/` and the byte layout of every object
type. External tools MUST be able to produce and consume these bytes
using only this document.

All integers are little-endian. All hashes are 32-byte BLAKE3.
...
There is no v0. mkit does not read earlier-format bytes.
...
(see [Packfile](/docs/spec/packfile)).
...
Mode `0x04` is new in v1 (previously absent; see below silent
...
- `timestamp: u32` → `u64`. Avoids 2106 overflow.
...
**NOT part of the commit signing bytes** (see [Signing](/docs/spec/signing) §3). They are
...
repeat source_count:
    [32 bytes upstream_id]                was project_id in earlier drafts
...
Pack-only. See [Delta](/docs/spec/delta) for the instruction format and [Packfile](/docs/spec/packfile)
...
and the signing bytes (see [Signing](/docs/spec/signing)). The bytes are identical in both
...
marker. See [Staging index](/docs/spec/staging-index) for the `.mkit/index` sidecar.
...
3. **Canonical single-file tree**: one entry
   `{name="README.md", mode=0x01, object_hash=<hash of §13.1>}`.
   Record both the serialised bytes and the resulting BLAKE3 digest.
