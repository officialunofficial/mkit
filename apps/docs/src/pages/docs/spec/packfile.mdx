...
The byte layout of a `.mkit`-produced packfile, used for transport
upload/download and for bundle exchange.
...
Payload is exactly the bytes you would get from [Objects](/docs/spec/objects)
...
[Objects](/docs/spec/objects) prologue). The reconstructed bytes are then hashed to
...
multipart upload, which is a v2 candidate.
...
(Note: the current mkit impl does not write/verify a trailer. A future version will
...
2. **Single-raw pack** containing the empty blob from [Objects](/docs/spec/objects) §13
