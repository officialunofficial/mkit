...
description: "Editorial conventions for mkit documentation: tone, structure, headings, code-fence rules, and link style."
---
...
- Correct: See the [pack file specification](/docs/spec/packfile) for
  the byte-level layout.
- Incorrect: The pack file specification is available [here](/docs/spec/packfile).
...
[`docs/INSTALL.md`](/docs/install) for the full set.
...
- **Object** — a content-addressed unit stored in the repository.
  ...
  [SPEC-OBJECTS](/docs/spec/objects)).
...
- **Transport** — the protocol that moves packs between
  ...
  `mkit+s3`, and `mkit+ssh` (see [SPEC-TRANSPORT](/docs/spec/transport)).
- **Signer** — a component that produces a signature over a payload
  ...
  `contrib/signers/` (see [SPEC-EXTERNAL-SIGNER](/docs/spec/external-signer)).
...
- **Statement** — the in-toto v1 payload that an attestation
  describes (subject + predicate). See [SPEC-ATTESTATIONS](/docs/spec/attestations).
...
- See [CONTRIBUTING.md](https://github.com/officialunofficial/mkit/blob/main/CONTRIBUTING.md) for the local-dev
