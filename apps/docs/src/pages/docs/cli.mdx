---
title: CLI reference
description: "User-facing reference for the mkit command: subcommands, config keys, URL schemes, exit codes, env vars, and signals."
---
...
[Objects](/docs/spec/objects),
[Staging index](/docs/spec/staging-index),
[Refs](/docs/spec/refs),
[Packfile](/docs/spec/packfile),
[Transport](/docs/spec/transport),
[Signing](/docs/spec/signing),
[FastCDC](/docs/spec/fastcdc),
[SSH security](/docs/ssh-security).
...
in lowercase hex per [Objects](/docs/spec/objects) §9. Accepted
shorthands at parse time:
...
See [Transport](/docs/spec/transport) for the wire protocol.
...
- **`NO_COLOR`** — if set (any value, including empty) ANSI color on
  stdout is suppressed. See [https://no-color.org](https://no-color.org).
