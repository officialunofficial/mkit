...
`docs/specs/SPEC-*` corpus, and the CLI reference &mdash; so they build against the real
...
| `list_crates` | The workspace crates with descriptions and paths. |
| `get_crate_readme` | A crate's README (for example `mkit-keystore`). |
...
| `get_spec` | One SPEC doc by name (for example `ATTESTATIONS`). |
...
verification), use the local stdio MCP server built into the CLI itself &mdash;
`mkit mcp [--repository <path>]` &mdash; documented in `docs/CLI.md` §"Agent
...
`McpAgent` and `@modelcontextprotocol/sdk`, backed by **D1** (FTS5: trigram for
...
**code** and the served **corpus** deploy on separate paths &mdash; two GitHub
Actions workflows plus Cloudflare Workers Builds:

| Path | Trigger | Does |
|---|---|---|
| `mcp.yml` | PR/push touching mcp or docs | Validate only (index, typecheck, test, and unit-project coverage). |
| Cloudflare **Workers Builds** (CF dashboard git integration) | Merge to `main` | Build and deploy the Worker **code** only &mdash; never seeds. Tool/`instructions` fixes reach agents without waiting for a release. Same model the web app uses; configured in the Cloudflare dashboard, not in this repo. |
| `mcp-release.yml` | Release tag `v*.*.*` (or dispatch) | Check out the **tag**, guard tag == workspace version, **seed that version into D1 if not already indexed**. Already-indexed versions are never touched; `workflow_dispatch` with `force: true` re-seeds a version *from its own tag* (indexer-fix escape hatch). Does **not** deploy the Worker &mdash; Workers Builds already shipped the code from `main`. |

Consequence: docs/SPEC/SKILL edits on `main` appear in the MCP **at the next
release** (by design &mdash; served docs match the released binary). The Worker code
...
> automatically splits any oversized file into an `INSERT` and `UPDATE … content =
> content || '<chunk>'` appends (the FTS trigger re-syncs after each), and hard-
> fails if any emitted statement would still exceed the cap &mdash; so `db:seed` can't
...
> `db:seed` each part &mdash; the delete-by-version and upsert makes re-seeding idempotent.
...
environment's deployment-branch policy must allow tags matching `v*.*.*` &mdash;
