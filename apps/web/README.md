...
required &mdash; `mkit-rpc` builds from its vendored generated sources by default.)

Bun is the package manager and script runner only &mdash; waku, vitest, and wrangler
...
is not &mdash; its from-source fallback fails offline and nothing here does
...
`scripts/cf-build.sh` (which bootstraps rustup and wasm-pack on the build image)
...
prerendered HTML or RSC payload &mdash; waku's SSG step otherwise reports success
