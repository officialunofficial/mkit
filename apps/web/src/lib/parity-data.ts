  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        cmd: 'blame',
        status: 'divergent',
        note: 'Supports -L line ranges, a [<rev>] argument, -w, -M/-C move/copy detection, and --ignore-rev fall-through. Move/copy and --ignore-rev attribution is merge-aware, traced to the true origin across every merge parent, including both -C cross-parent tie-break rules (interior vs. boundary). No --porcelain; --format=json carries an mkit Identity, not Name <email>.',
      },
      {
        cmd: 'bisect',
        status: 'divergent',
