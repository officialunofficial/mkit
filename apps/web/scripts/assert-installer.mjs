...
// via env.ASSETS). /install.ps1 is the native-Windows PowerShell counterpart
// (`irm https://mkit.sh/install.ps1 | iex`) — no bare-domain UA sniff for it,
// just the explicit static path. Both are staged by scripts/stage-public.mjs
// BEFORE `waku build` collects static assets; if that step is skipped/
// reordered or an asset is pruned, the build still succeeds but the installer
// silently 404s. Run after `waku build`.
...
console.log('assert-installer: ok (dist/public/install.sh, dist/public/install.ps1 shipped)')
