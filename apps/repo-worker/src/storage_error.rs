...
// plain `cargo test --lib`, the same reasoning `hashing::constant_time_eq`
// documents for why it isn't defined inside `worker_impl` either.
...
pub enum StorageOp {
    ...
    R2Read,
    /// An R2 `list` (paging a prefix) failed.
    R2List,
    /// An R2 `delete_multiple` failed.
    R2Delete,
    ...
    /// failed.
    RefstoreStub,
    /// Building the JSON POST (or WebSocket-upgrade GET) request to the DO
    /// failed — request construction, header set, or body serialization.
    RefstoreRequest,
    ...
    RefstoreDecode,
    /// Opening the `/watch` WebSocket-upgrade request to the DO failed.
    RefstoreWatchFetch,
    /// Accepting the upgraded WebSocket, or the DO not upgrading at all,
    /// failed.
    RefstoreWatchAccept,
    /// The WatchRefs bridge's own socket event stream (`ws.events()`) or an
    /// in-stream socket error.
    WatchSocket,
...
impl StorageOp {
    ...
    const ALL: &'static [StorageOp] = &[
        ...
        StorageOp::RefstoreWatchFetch,
        StorageOp::RefstoreWatchAccept,
        StorageOp::WatchSocket,
    ...
    fn label(self) -> &'static str {
        match self {
            ...
            StorageOp::R2List => "R2 list",
            StorageOp::R2Delete => "R2 delete_multiple",
            ...
            StorageOp::RefstoreWatchFetch => "REFSTORE watch fetch",
            StorageOp::RefstoreWatchAccept => "REFSTORE watch accept",
            StorageOp::WatchSocket => "REFSTORE watch socket",
    ...
    fn client_message(self) -> &'static str {
        match self {
            ...
            | StorageOp::R2Read
            | StorageOp::R2List
            | StorageOp::R2Delete => "object storage request failed",
            StorageOp::RefstoreBinding
            | StorageOp::RefstoreStub
            | StorageOp::RefstoreRequest
            | StorageOp::RefstoreFetch
            ...
            | StorageOp::RefstoreWatchFetch
            | StorageOp::RefstoreWatchAccept
            | StorageOp::WatchSocket => "ref store request failed",
...
mod tests {
    ...
    fn simulated_storage_failure_is_logged_but_never_reaches_the_client() {
        let raw_detail = "R2Error: bucket 'mkit-prod-storage-7f3a' access denied \
        ...
        for &op in StorageOp::ALL {
            ...
            assert!(
                !client_msg.contains("mkit-prod-storage-7f3a"),
