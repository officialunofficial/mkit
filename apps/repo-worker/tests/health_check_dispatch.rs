...
// dispatch — the exact machinery `apps/repo-worker/src/worker_impl.rs`'s
// `serve_connect` uses in production — over a genuine Connect unary HTTP
// request/response. Mirrors `tests/purge_room_dispatch.rs`'s pattern and
// rationale for why this is host-only and needs neither the wasm32 target
// nor a live R2/Durable-Object runtime.
//
...
use mkit_repo_worker::proto::grpc::health::v1::{
...
async fn health_check_reports_serving_under_normal_conditions() {
    let service = FakeHealthService {
        known_service: "mkit.repo.v1.RepoService",
    ...
    // The registered RepoService by name.
    let resp = check_over_wire(&service, "mkit.repo.v1.RepoService").await;
    assert_eq!(resp["status"], "SERVING");
}
