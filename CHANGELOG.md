...
- *(core)* `ops::diff::myers_changed` (the line-level Myers edit script behind `unified_hunks`/`enumerate_hunks` — `diff`, `show`, `add -p`, and `merge_blob_3way`'s 3-way diff) now elides any common leading/trailing run of identical lines before running its O(ND) core on the (usually much smaller) middle region that actually differs — the same "diff only the changed region" step git's `xdiff` and every other practical diff engine apply before their own O(ND) search. A real edit almost always leaves most of a file as a shared prefix and/or suffix (a single changed line, an appended block); the old code always sized its `v`/backtrack-`trace` state off the *whole* `old.len() + new.len()`, regardless of how small the actual edit was. New elision logic in `myers_changed` finds the shared affix with two cheap linear scans (bounded by `min(old.len(), new.len())`, so it can never overlap), runs the renamed `myers_changed_core` (the previous function body, unchanged) on just the middle slices, and splices the result back into full-sized change-flag vectors — the elided run stays unmarked either way, since `compact_changes`/`script_from_flags` already pair unmarked lines in order and a greedy Myers walk would have matched that same run first/last on its own; this is a pure speedup, not a behavior change (pinned by a new `proptest_hunks_roundtrip_with_shared_affix` property test: for a random shared prefix/suffix plus a random differing middle, applying every `enumerate_hunks` hunk reconstructs `new` from `old` byte-for-byte, and applying none reconstructs `old`).

  New `cargo bench -p mkit-benches --bench diff_edit_script` suite (single-line edit and 200-line append against 1k/10k/100k-line files, plus a `no_common_affix` control with no shared prefix or suffix at all, 4-core host, `--quick`): 10k-line append 27.87ms → 0.21ms (132x), 100k-line append 297.4ms → 4.11ms (72x) — the appended-block case is the flagship win, since the entire original file elides as a shared prefix and only the new tail ever reaches the O(ND) core; 100k-line single-line edit 9.04ms → 5.20ms (−42%, the remaining cost is `split_lines`/`compact_changes`/`script_from_flags`'s own O(n) work, which this change doesn't touch). `no_common_affix` (1k/2k lines, kept small since it's `O((n+m)^2)` independent of this change — Myers' own worst case, a genuinely unrelated cost) is unaffected within run-to-run noise (33-40ms/170-178ms both before and after), confirming the two extra linear scans cost nothing when there's no shared affix to find. All 860 `mkit-core` tests pass (including the new property test), `cargo clippy -p mkit-core -p mkit-benches --all-targets -- -D warnings` and `cargo fmt --check` are clean. **SemVer:** none — `myers_changed`/`myers_changed_core` are private to `ops::diff`; every public diff/merge API's behavior and output are unchanged.
