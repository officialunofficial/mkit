...
- *(core)* eliminate a redundant first-parent-chain walk and MMR rebuild from every `history-mmr` ref publish (issue #644's ancestry-snapshot feature, default-off). `ancestry::advance` already rebuilt the target's `AncestrySnapshot` once to validate it before persisting a `transaction` intent record, then discarded that result; `ancestry::finish` walked the identical chain from the object store and rebuilt the MMR a second time from scratch to actually persist it. Both walks/builds are over the same verified, content-addressed chain under the same history+ref locks held for the whole call, so the first build is provably still valid when `finish` needs it: `advance` now keeps it and passes it through a new `prebuilt: Option<AncestrySnapshot>` parameter, which `finish` uses only when its descriptor's repository/full_ref/generation/tip all match the transaction being finished (a `debug_assert` catches a mismatched `Some` reaching that fallback, which should be unreachable). The two call sites that recover a stale on-disk transaction left by a prior crashed process (`recover`, and `advance`'s own leftover-transaction check) are unchanged and still pass `None`, forcing a from-scratch rebuild. Separately, `CommitHistory` (the in-memory MMR wrapper) gains `extend`, which merkleizes a whole run of leaf hashes as a single batch instead of one batch per leaf; `AncestrySnapshot::build` now calls it once instead of looping `append`. `cargo bench -p mkit-benches --features history-mmr --bench history_mmr` (10-sample criterion, this branch's 4-core host): `publish/50` 9.93ms → 6.81ms (−31%), `publish/250` 19.08ms → 15.52ms (−19%), `load/50` 335.0µs → 317.9µs (−5%), `load/250` 1.876ms → 1.671ms (−11%); criterion independently reported all four as statistically significant (p<0.05) when comparing the two runs. New `history::tests::extend_matches_sequential_append_root_and_positions` pins `extend`'s batched output equal to sequential `append` (root and leaf count) across MMR power-of-two boundaries; the existing `sequential_fast_forward_and_backfill_have_identical_roots_and_positions` and `every_publication_boundary_recovers_the_whole_fast_forward` fault-injection tests already covered the `finish` reuse and stale-recovery paths respectively and continue to pass. **SemVer:** additive — `mkit-core` gains `history::CommitHistory::extend`; `finish` is a private function, so its new parameter is not part of the public API. No behavior change to any public function's inputs or outputs.
