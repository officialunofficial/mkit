...
- *(core)* replace `delta::encode`'s block-hash index (`HashMap<u64, u32>`, `std`'s default `SipHash`-1-3 hasher) with the same map keyed by a new local `FxHasher`. The index's keys are already the output of `encode`'s own FNV-1a `block_hash` over each 16-byte block of `base` — re-hashing an already-well-distributed 64-bit value with a cryptographically-DoS-resistant keyed hash buys nothing: an attacker who controls `base`'s bytes already controls `block_hash`'s output directly, so `SipHash`'s per-byte cost on top of it is pure overhead with no corresponding security benefit. `FxHasher` is the same class of hasher as the `rustc-hash`/`fxhash` crates (the mix step — `(state.rotate_left(5) ^ word) * SEED`, `SEED` `FxHash`'s own odd constant — is `FxHash`'s), reimplemented locally (a dozen lines, `delta.rs`'s only user) rather than taken as a new workspace dependency. `cargo bench -p mkit-benches --bench delta_plan_fanout -- --quick` (sequential series, 4-core host, near-duplicate 64 KiB chunks): 32 candidates 3.25ms → 2.48ms, 64 8.50-8.65ms → 5.07-5.13ms, 128 14.0-17.0ms → 8.86-11.8ms, 256 candidates 36.6-37.1ms → 22.6-23.5ms (~36-39% faster) — `delta::encode` is CPU-bound in exactly this hash-table build-and-probe loop, so the win holds across both the sequential path and `remote_dispatch`'s existing rayon fan-out over it (256 candidates, rayon series: 10.9ms → ~8.0ms). New `fx_hasher_is_deterministic_and_avalanches` test pins `FxHasher`'s two load-bearing properties directly (same input → same output; a one-bit input change flips roughly half the output bits) plus exercises the generic byte-slice `write` path (unused by `encode`'s `u64` keys today, but implemented so the hasher stays correct if ever reused for a different key type). No wire-format or public-API change — `FxHasher`/`FxIndexMap` are private to `delta.rs`, and `encode`'s output is byte-for-byte identical to before (all 786 existing `mkit-core` tests, including `delta`'s roundtrip and corruption-rejection suite, pass unchanged). **SemVer:** none.
