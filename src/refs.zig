...
pub fn listRefs(allocator: Allocator, io: Io, dir: Io.Dir) ![]Ref {
    return listRefsFromDir(allocator, io, dir, heads_dir);
}

/// Delete a branch ref. Returns error if ref doesn't exist.
pub fn deleteRef(io: Io, dir: Io.Dir, branch: []const u8) !void {
    if (!protocol.validateRefName(branch)) return error.InvalidRefName;
    deleteRefFromDir(io, dir, heads_dir, branch) catch |err| switch (err) {
...
pub fn deleteRefSafe(allocator: Allocator, io: Io, dir: Io.Dir, branch: []const u8) !void {
    const head = readHead(allocator, io, dir) catch return deleteRef(io, dir, branch);
    switch (head) {
        .branch => |current_branch| {
            ...
            return deleteRef(io, dir, branch);
        },
        .detached => return deleteRef(io, dir, branch),
    }
}
...
pub fn init(io: Io, dir: Io.Dir) !void {
    dir.createDirPath(io, refs_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    };
    dir.createDirPath(io, heads_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    };
    dir.createDirPath(io, tags_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    };
    // Create default HEAD pointing to main
    const f = dir.createFile(io, head_file, .{ .exclusive = true }) catch |err| switch (err) {
...
pub fn readHead(allocator: Allocator, io: Io, dir: Io.Dir) !Head {
    const content = dir.readFileAlloc(io, head_file, allocator, .limited(4096)) catch return error.NoHead;
...
pub fn writeHeadBranch(io: Io, dir: Io.Dir, branch: []const u8) !void {
    if (!protocol.validateRefName(branch)) return error.InvalidRefName;
    var buf: [128]u8 = undefined;
    const data = try std.fmt.bufPrint(&buf, "ref: refs/heads/{s}\n", .{branch});
    try writeAtomicFile(io, dir, head_file, data, false);
}

/// Write HEAD as a detached hash.
pub fn writeHeadDetached(io: Io, dir: Io.Dir, h: Hash) !void {
...
pub fn readRef(allocator: Allocator, io: Io, dir: Io.Dir, branch: []const u8) !?Hash {
    if (!protocol.validateRefName(branch)) return error.InvalidRefName;
    var heads = dir.openDir(io, heads_dir, .{}) catch return null;
    defer heads.close(io);
    return readRefFromDir(allocator, io, heads, branch) catch return null;
}

/// Read a ref hash from an already-opened directory handle.
fn readRefFromDir(allocator: Allocator, io: Io, heads: Io.Dir, branch: []const u8) !?Hash {
    const content = heads.readFileAlloc(io, branch, allocator, .limited(128)) catch return null;
    defer allocator.free(content);
...
pub fn writeRef(io: Io, dir: Io.Dir, branch: []const u8, h: Hash) !void {
    if (!protocol.validateRefName(branch)) return error.InvalidRefName;
    return writeRefToDir(io, dir, heads_dir, branch, h);
}

/// Resolve HEAD to a commit hash. Returns null if HEAD's branch has no commits yet.
pub fn resolveHead(allocator: Allocator, io: Io, dir: Io.Dir) !?Hash {
    const head = readHead(allocator, io, dir) catch return null;
    switch (head) {
        .branch => |branch| {
            defer allocator.free(branch);
            return try readRef(allocator, io, dir, branch);
        },
        .detached => |h| return h,
    }
}
...
pub fn updateHead(allocator: Allocator, io: Io, dir: Io.Dir, commit_hash: Hash) !void {
    const head = try readHead(allocator, io, dir);
    switch (head) {
        .branch => |branch| {
            defer allocator.free(branch);
            try writeRef(io, dir, branch, commit_hash);
        },
        .detached => {
            try writeHeadDetached(io, dir, commit_hash);
        },
    }
}
...
pub fn writeTag(io: Io, dir: Io.Dir, name: []const u8, h: Hash) !void {
    if (!protocol.validateRefName(name)) return error.InvalidRefName;
    return writeRefToDir(io, dir, tags_dir, name, h);
}

/// Read the hash that a tag points to. Returns null if tag doesn't exist.
pub fn readTag(allocator: Allocator, io: Io, dir: Io.Dir, name: []const u8) !?Hash {
    if (!protocol.validateRefName(name)) return error.InvalidRefName;
    var tags = dir.openDir(io, tags_dir, .{}) catch return null;
    defer tags.close(io);
    return readRefFromDir(allocator, io, tags, name) catch return null;
}

/// Delete a tag. Returns error if tag doesn't exist.
pub fn deleteTag(io: Io, dir: Io.Dir, name: []const u8) !void {
    if (!protocol.validateRefName(name)) return error.InvalidRefName;
    deleteRefFromDir(io, dir, tags_dir, name) catch |err| switch (err) {
...
pub fn listTags(allocator: Allocator, io: Io, dir: Io.Dir) ![]Ref {
    return listRefsFromDir(allocator, io, dir, tags_dir);
}

// -- Shared internal helpers --

fn writeRefToDir(io: Io, dir: Io.Dir, sub_dir: []const u8, name: []const u8, h: Hash) !void {
    ...
    try writeAtomicFile(io, dir, path, data, true);
}

fn deleteRefFromDir(io: Io, dir: Io.Dir, sub_dir: []const u8, name: []const u8) !void {
    var path_buf: [256]u8 = undefined;
    const path = std.fmt.bufPrint(&path_buf, "{s}/{s}", .{ sub_dir, name }) catch return error.NameTooLong;
...
fn listRefsFromDir(allocator: Allocator, io: Io, dir: Io.Dir, sub_dir: []const u8) ![]Ref {
    ...
    var sub = dir.openDir(io, sub_dir, .{ .iterate = true }) catch |err| switch (err) {
        error.FileNotFound => return ref_list.toOwnedSlice(allocator),
        else => return err,
    };
    sub.close(io);

    try collectRefsRecursive(allocator, io, dir, sub_dir, "", &ref_list);
...
fn writeAtomicFile(io: Io, dir: Io.Dir, path: []const u8, data: []const u8, make_path: bool) !void {
    // Why: 0.16 replaces dir.atomicFile with createFileAtomic + File.Atomic.replace/link.
...
fn collectRefsRecursive(
    ...
    var current = root.openDir(io, dir_path, .{ .iterate = true }) catch |err| switch (err) {
    ...
    while (try iter.next(io)) |entry| {
        ...
        switch (entry.kind) {
            .file => {
                ...
                const ref_hash = readRefFromDir(allocator, io, current, entry.name) catch {
                    // Why: readRefFromDir only yields error.InvalidRef; all other failures map to null.
                    allocator.free(name);
                    continue;
                };
                try ref_list.append(allocator, .{ .name = name, .hash = ref_hash });
            },
            .directory => try collectRefsRecursive(allocator, io, root, sub_dir, child_name, ref_list),
            else => {},
        }
    }
}
...
pub fn loadShallowBoundaries(allocator: Allocator, io: Io, mkit_dir: Io.Dir) !?std.AutoHashMap(Hash, void) {
    const content = mkit_dir.readFileAlloc(io, shallow_file, allocator, .limited(1024 * 1024)) catch |err| switch (err) {
...
pub fn writeShallowBoundaries(io: Io, mkit_dir: Io.Dir, boundaries: []const Hash) !void {
    if (boundaries.len == 0) {
        mkit_dir.deleteFile(io, shallow_file) catch |err| switch (err) {
            error.FileNotFound => {},
            else => return err,
        };
        return;
    ...
    var atomic_file = try mkit_dir.createFileAtomic(io, shallow_file, .{
        .make_path = true,
        .replace = true,
    });
    defer atomic_file.deinit(io);
    ...
    for (boundaries) |h| {
        const hex = hash_mod.toHex(h);
        try file_writer.interface.writeAll(&hex);
...
test "init creates refs structure and HEAD" {
    ...
    const head = try readHead(allocator, io, tmp.dir);
    switch (head) {
        .branch => |branch| {
            defer allocator.free(branch);
...
test "write and read branch ref" {
    ...
    try writeRef(io, tmp.dir, "main", h);

    const read = try readRef(allocator, io, tmp.dir, "main");
...
test "resolve HEAD with no commits returns null" {
    ...
    const resolved = try resolveHead(allocator, io, tmp.dir);
...
test "update HEAD updates current branch" {
    ...
    try updateHead(allocator, io, tmp.dir, h1);
    ...
    try updateHead(allocator, io, tmp.dir, h2);

    const resolved2 = try resolveHead(allocator, io, tmp.dir);
...
test "detached HEAD" {
    ...
    try writeHeadDetached(io, tmp.dir, h);
...
test "nonexistent branch returns null" {
    ...
    const result = try readRef(allocator, io, tmp.dir, "nonexistent");
...
test "list refs empty" {
    ...
    const ref_list = try listRefs(allocator, io, tmp.dir);
    defer {
        for (ref_list) |ref| {
            allocator.free(ref.name);
        }
...
test "list refs after writes" {
    ...
    try writeRef(io, tmp.dir, "main", h1);
    try writeRef(io, tmp.dir, "dev", h2);
...
test "delete ref" {
    ...
    try writeRef(io, tmp.dir, "feature", h);

    // Verify it exists
    const before = try readRef(allocator, io, tmp.dir, "feature");
    try std.testing.expectEqual(h, before.?);

    // Delete it
    try deleteRef(io, tmp.dir, "feature");

    // Verify it's gone
    const after = try readRef(allocator, io, tmp.dir, "feature");
...
test "delete nonexistent ref" {
    ...
    const result = deleteRef(io, tmp.dir, "nonexistent");
...
test "checkout switches HEAD" {
    ...
    // Switch HEAD to "dev"
    try writeHeadBranch(io, tmp.dir, "dev");
...
test "refuse delete current branch" {
    ...
    const result = deleteRefSafe(allocator, io, tmp.dir, "main");
...
test "write and read tag" {
    ...
    try writeTag(io, tmp.dir, "v1.0", h);

    const read = try readTag(allocator, io, tmp.dir, "v1.0");
    try std.testing.expectEqual(h, read.?);
}
...
test "list tags empty" {
    ...
    const tag_list = try listTags(allocator, io, tmp.dir);
    defer {
        for (tag_list) |t| {
...
test "list tags sorted" {
    ...
    try writeTag(io, tmp.dir, "v2.0", h1);
    try writeTag(io, tmp.dir, "v1.0", h2);
    try writeTag(io, tmp.dir, "alpha", h3);
...
test "delete tag" {
    ...
    try writeTag(io, tmp.dir, "release", h);

    // Verify it exists
    const before = try readTag(allocator, io, tmp.dir, "release");
    try std.testing.expectEqual(h, before.?);

    // Delete it
    try deleteTag(io, tmp.dir, "release");

    // Verify it's gone
    const after = try readTag(allocator, io, tmp.dir, "release");
    try std.testing.expectEqual(@as(?Hash, null), after);
}
...
test "delete nonexistent tag" {
    ...
    const result = deleteTag(io, tmp.dir, "nonexistent");
...
test "tag and branch same name" {
    ...
    try writeTag(io, tmp.dir, "main", tag_hash);
    try writeRef(io, tmp.dir, "main", branch_hash);

    // Both should be independently readable
    const read_tag = try readTag(allocator, io, tmp.dir, "main");
    const read_branch = try readRef(allocator, io, tmp.dir, "main");
...
test "reject invalid branch names" {
    ...
    try std.testing.expectError(error.InvalidRefName, writeRef(io, tmp.dir, "../escape", h));
    try std.testing.expectError(error.InvalidRefName, writeHeadBranch(io, tmp.dir, "bad//branch"));
}
...
test "nested refs are listed recursively" {
    ...
    try writeRef(io, tmp.dir, "feature/deep/topic", h);

    const refs = try listRefs(allocator, io, tmp.dir);
    defer {
...
test "loadShallowBoundaries returns null when file missing" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io, ".mkit");
    const result = try loadShallowBoundaries(allocator, io, tmp.dir);
...
test "writeShallowBoundaries and loadShallowBoundaries roundtrip" {
    ...
    try writeShallowBoundaries(io, tmp.dir, &boundaries);
    var loaded = (try loadShallowBoundaries(allocator, io, tmp.dir)).?;
    defer loaded.deinit();
...
test "writeShallowBoundaries empty removes file" {
    ...
    var loaded = (try loadShallowBoundaries(allocator, io, tmp.dir)).?;
    loaded.deinit();
    try writeShallowBoundaries(io, tmp.dir, &[_]Hash{});
    const result = try loadShallowBoundaries(allocator, io, tmp.dir);
    try std.testing.expect(result == null);
}
...
test "loadShallowBoundaries empty file returns null" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io, ".mkit");
    const f = try tmp.dir.createFile(io, shallow_file, .{});
    f.close(io);
    const result = try loadShallowBoundaries(allocator, io, tmp.dir);
    try std.testing.expect(result == null);
}
...
test "loadShallowBoundaries skips invalid lines" {
    ...
    try f.writeStreamingAll(io, &hex);
    try f.writeStreamingAll(io, "\n");
    try f.writeStreamingAll(io, "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz\n");
    try f.writeStreamingAll(io, "\n");
    var loaded = (try loadShallowBoundaries(allocator, io, tmp.dir)).?;
    defer loaded.deinit();
